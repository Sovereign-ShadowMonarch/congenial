...
// Create a simple TradingViewWidget component that works with React 18
class TradingViewWidget extends React.Component {
  ...
  initWidget() {
    if (window.TradingView && this.containerRef) {
      new window.TradingView.widget({
        autosize: true,
        symbol: this.props.symbol,
        interval: this.props.interval || '5',
        timezone: this.props.timezone || 'Asia/Kolkata',
        ...
        container_id: this.containerRef.id,
      });
    }
  }
...
class Coin extends React.Component {
  ...
  componentDidMount() {
    if (this.state.coin !== undefined) {
      ...
      // Add price refresh every 10 seconds for real-time updates
      this.priceInterval = setInterval(() => {
        this.getAPIData(this.state.coin);
        console.log("Refreshed price data for", this.state.coin);
      }, 10000);
    }
  }
