...
class ExchangeInterface(CacheableObject, LockableQueryObject):
    ...
    def query_margin_history(
        ...
        """Queries the local DB and the remote exchange for the margin positions
        history of the user
        """
    ...
    def query_history_with_callbacks(
        ...
        try:
            ...
            exchange_specific_data = self.query_exchange_specific_history(  # pylint: disable=assignment-from-none
                start_ts=start_ts,
                end_ts=end_ts,
            )
            success_callback(
